...
Built on the [Mistral Search Toolkit](https://pypi.org/project/mistralai-search-toolkit/) SDK — ingestion pipelines, Vespa indexing, and hybrid retrieval.
...
Initial setup asks for **Mistral API key**, **collection name**, and **deployment name**. The **destination folder** is the project name (`pyproject.toml`, README, Vespa container). Ports default to `18080` / `19072` in `.env`.
...
└── src/
    ...
    ├── search_app/
    │   ├── __init__.py             # VespaApp / PostgresApp definition
    │   └── migrations/             # mistral-vespa migrate (hybrid query profile)
    └── examples/
        └── workflows/              # Search ingestion workflows (plugin-based)
```
...
| `deployment_name`  | Mistral Workflows deployment name (written to `.env` as `DEPLOYMENT_NAME`) |
