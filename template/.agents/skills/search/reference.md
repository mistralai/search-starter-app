...
Separate wheels (install explicitly): `mistralai-search-toolkit-storage-s3`, `mistralai-search-toolkit-plugins-vespa`, etc.
...
|----------|---------|------|
| `MISTRAL_API_KEY` | empty until `make installdeps` | Set in `.env` when exported before `make installdeps` |
...
| `COLLECTION_NAME` | `exampledocs` | Vespa schema collection; must match `src/vespa_app/migrations/` |
